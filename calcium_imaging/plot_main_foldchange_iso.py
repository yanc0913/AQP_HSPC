...
def draw_panel(ax, P, ylim, ref, ylab, title):
    ...
    for sx in P["seps"]:
        ax.axvline(sx, color="0.5", ls=":", lw=1.0, zorder=0)
    ax.set_xticks(P["xpos"])
    ax.set_xticklabels(P["labels"], rotation=20, ha="right")
    ax.set_ylabel(ylab)
...
def main():
    ...
    print(f"[OK] wrote combined + 4 individual panels + workbook to {OUT_DIR}")
